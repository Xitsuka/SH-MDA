...
class MDA():
    ...
    def train(self):
        ...
        for i in progress_bar:
            ...
            domin_params = [p for n, p in self.model.named_parameters() if 'domin' in n]
            ...
            optimizer_c=torch.optim.Adam([{'params': domin_params, 'lr': LEARNING_RATE/10}])
            domain_data=[]
            domain_label=[]
    ...
    def test(self):
        ...
        with torch.no_grad():
            for data, target in self.target_loader:
                ...
                domain_pred = torch.round(F.softmax(self.model.domin(data)/10, dim=1)*100)/100
